...
def findSecondMeasurement(firstMeasurement, standardDeviation):
    ...
    diff = lisa - susan
    meann = (lisa + susan) / 2
    first = diff / standardDeviation
    second = (firstMeasurement - meann) / standardDeviation
    temp = exp(first * second)

    secondMeasurement = susan + (lisa - susan) * ((temp) / (1 + temp))  # Replace me with your answer
...
if __name__ == "__main__":
    ...
    print('For First Measurement %.2f, standard deviation %.2f, Expected second measurement: %.2f' % (
        firstMeasurement, standardDeviation \
            , findSecondMeasurement(firstMeasurement, standardDeviation)))
